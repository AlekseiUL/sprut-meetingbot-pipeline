...
> This backend is included as a derived component of Sprut MeetingBot Pipeline. For the full local Whisper/pyannote pipeline and Mac mini setup, start from the repository root `README.md` and `docs/LOCAL_MAC_SETUP.md`.
>
> Upstream backend origin: https://github.com/screenappai/meeting-bot (MIT).
...
#### Using the local Docker Compose backend

For Sprut MeetingBot Pipeline, build the image locally from this repository:

```bash
cd backend
docker compose up -d --build
# or, on systems with the standalone plugin:
docker-compose up -d --build
curl -sS http://127.0.0.1:3001/health
```

Do not publish images built from a working tree that contains `.env`, auth state, recordings, transcripts, screenshots, or debug artifacts.
