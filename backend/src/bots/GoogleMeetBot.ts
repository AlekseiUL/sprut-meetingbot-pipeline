...
export class GoogleMeetBot extends MeetBotBase {
  ...
  private async joinMeeting({ url, name, teamId, userId, eventId, botId, pushState, uploader }: JoinParams & { pushState(state: BotStatus): void }): Promise<void> {
    ...
    this._logger.info('Waiting briefly for Google Meet pre-join UI...');
    ...
    this._logger.info('Waiting briefly before filling the name...');
    ...
    this._logger.info('Waiting briefly before joining...');
    await this.page.waitForTimeout(1000);
    ...
    pushState('joined');
    await this.captureParticipantsSnapshot({ userId, teamId, eventId, botId });
  ...
  private async recordMeetingPage(
    ...
    await this.page.evaluate(
      async ({ teamId, duration, inactivityLimit, userId, slightlySecretId, activateInactivityDetectionAfter, activateInactivityDetectionAfterMinutes, primaryMimeType, secondaryMimeType, silenceThreshold, participantInactivityEnabled }: 
      { teamId:string, userId: string, duration: number, inactivityLimit: number, slightlySecretId: string, activateInactivityDetectionAfter: string, activateInactivityDetectionAfterMinutes: number, primaryMimeType: string, secondaryMimeType: string, silenceThreshold: number, participantInactivityEnabled: boolean }) => {
        ...
        async function startRecording() {
           ...
           */
          if (participantInactivityEnabled) {
            ...
            console.log('Participant inactivity auto-exit is disabled for Google Meet.');
          }
      ...
      { 
        ...
        silenceThreshold: config.meetingSilenceThreshold,
        participantInactivityEnabled: config.participantInactivityEnabled,
      }
    );
  ...
  private async captureParticipantsSnapshot(
    { userId, teamId, eventId, botId }:
    { userId: string, teamId: string, eventId?: string, botId?: string }
  ): Promise<void> {
    if (!this.page) return;

    try {
      const collectNames = async () => await this.page.evaluate(() => {
        const ignored = [
          'keep_outline',
          'more_vert',
          'mic_',
        ...
        const addCandidate = (raw: string | null | undefined) => {
          ...
          if (ignored.some((item) => text.includes(item))) return;
          if (/^(you|me|presentation|presenting|screen|settings|captions|camera|microphone)$/i.test(text)) return;
        ...
        for (const selector of targetedSelectors) {
          for (const element of Array.from(document.querySelectorAll(selector))) {
            ...
            addCandidate(html.getAttribute('data-self-name'));
            addCandidate(html.getAttribute('aria-label'));
            addCandidate(html.innerText || html.textContent || '');
          }
        }

        // Google Meet keeps tile labels in obfuscated div/span classes.
        // The People panel is not enough: visible speaking tiles can contain names
        // that never appear in the panel scrape.
      ...
      const visibleTileParticipants = await collectNames();

      const peopleButton = this.page.locator('button[aria-label^="People"], button[aria-label*="People"]').first();
      if (await peopleButton.count()) {
        await peopleButton.click({ timeout: 5000 }).catch(() => undefined);
        await this.page.waitForTimeout(1000);
      }

      const panelParticipants = await collectNames();
      const participants = Array.from(new Set([...visibleTileParticipants, ...panelParticipants]));

      if (!participants.length) {
        this._logger.warn('Participants snapshot is empty', { userId, teamId, eventId, botId });
        return;
      }

      for (const participant of participants) {
        this._logger.info(`Participant detected: ${participant}`, { userId, teamId, eventId, botId });
      }

      const debugDir = path.join(process.cwd(), 'debug-videos');
      await fs.promises.mkdir(debugDir, { recursive: true });
      const filePath = path.join(debugDir, `participants-${Date.now()}.json`);
      await fs.promises.writeFile(
        filePath,
        JSON.stringify({
          participants,
          visibleTileParticipants,
          panelParticipants,
      ...
      this._logger.info('Participants snapshot saved', { filePath, count: participants.length, userId, teamId });
    } catch (error) {
      this._logger.warn('Failed to capture participants snapshot', { error, userId, teamId, eventId, botId });
    }
  }
}
