...
class DiskUploader implements IUploader {
  ...
  public async uploadRecordingToRemoteStorage(options?: { forceUpload?: boolean }) {
    try {
      ...
      } else if (config.uploaderType === 'local') {
        this._logger.info('Local uploader mode enabled. Keeping recording on disk for host monitor.', {
          userId: this._userId,
          teamId: this._teamId,
          filePath: DiskUploader.getFilePath(this._userId, this._tempFileId, this.fileExtension),
        });
        return true;
      } else {
